...
import { AlertTriangle, Boxes, PlusMinus } from 'lucide-react'
  ...
  return <>
    ...
    <div className="panel module-panel"><div className="panel-toolbar"><div><h3>Lotes e saldos</h3><p>Consulte custo, validade, local e quantidade disponível.</p></div></div>
      {loading?<LoadingPanel text="Carregando estoque…"/>:lots.length===0?<div className="empty"><div className="empty-icon"><Boxes/></div><h3>Estoque vazio</h3><p>Confirme uma compra para criar os primeiros lotes e movimentações.</p></div>:<DataTable headers={['Insumo','Marca / Lote','Local','Saldo','Custo','Valor','Validade','Ações']}>
        {lots.map(l=><tr key={l.stock_lot_id} className={Number(l.available_quantity)<=0?'row-muted':''}><td><strong>{l.ingredient_name||'Item'}</strong><small className="cell-helper">Recebido: {numberBR.format(Number(l.quantity_received))} {l.unit_symbol||''}</small></td><td>{l.brand_name||'Sem marca'}<small className="cell-helper">Lote: {l.lot_code||'—'}</small></td><td>{l.location_name||'—'}</td><td><strong>{numberBR.format(Number(l.available_quantity))} {l.unit_symbol||''}</strong></td><td>{brl.format(Number(l.unit_cost_base))}/{l.unit_symbol||'un'}</td><td>{brl.format(Number(l.stock_value))}</td><td>{expiryBadge(l.expires_at)}</td><td><button className="table-action" onClick={()=>setAdjustLot(l)}><PlusMinus size={15}/> Ajustar</button></td></tr>)}
      </DataTable>}
    </div>
    <div className="panel module-panel"><div className="panel-toolbar"><div><h3>Movimentações recentes</h3><p>Últimos 50 lançamentos do livro-razão.</p></div></div>{movements.length===0?<div className="panel-empty">Nenhuma movimentação registrada.</div>:<DataTable headers={['Data','Tipo','Quantidade','Custo unitário','Referência']}>
      {movements.map(m=>{const lot=lots.find(l=>l.stock_lot_id===m.stock_lot_id);return <tr key={m.id}><td>{dateTimeBR(m.created_at)}</td><td><Badge tone={Number(m.quantity)>=0?'success':'danger'}>{movementLabel(m.movement_type)}</Badge></td><td>{Number(m.quantity)>0?'+':''}{numberBR.format(Number(m.quantity))} {lot?.unit_symbol||''}</td><td>{brl.format(Number(m.unit_cost))}</td><td>{m.reference_type}</td></tr>})}
    </DataTable>}</div>

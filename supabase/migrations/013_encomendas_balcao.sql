-- MoveAI SmartPDV
-- 013_encomendas_balcao.sql
-- Adiciona "balcao" como forma de atendimento para encomendas.
-- Mantém channel='encomenda' para o pedido continuar visível no módulo Encomendas.
...
alter table public.orders
  ...
  check (fulfillment_type in ('retirada','entrega','balcao'));
...
begin
  ...
  if p_fulfillment_type not in ('retirada','entrega','balcao') then
    raise exception 'Forma de atendimento inválida';
  end if;

  if p_scheduled_for is null then
    raise exception 'Informe a data e hora prevista';
  end if;
  ...
  values(
    ...
    p_scheduled_for,
    case
      when p_fulfillment_type = 'entrega'
      then nullif(trim(coalesce(p_delivery_address, '')), '')
      else null
    end,
...
select
  '013_encomendas_balcao aplicada com sucesso' as status,
  now() as applied_at;
